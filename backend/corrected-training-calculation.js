...
const testTrainingCalculation = async () => {
    try {
        ...
        console.log('=== CORRECTED TRAINING CALCULATION TEST ===');
        
        // Find the admin with 19 branches (javad)
        const admin = await Admin.findOne({ name: 'javad' }).populate('branches');
        ...
        console.log(`Admin has access to ${admin.branches.length} branches:`, admin.branches.map(b => b.locCode));
